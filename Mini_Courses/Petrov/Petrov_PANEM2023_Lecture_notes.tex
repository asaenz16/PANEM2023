...
The basic coupling can be generalized to an arbitrary number of ASEPs,
and this leads to a colored ASEP
...
We see that lower colors has less jumping priority if $q<1$.
...
$<k$ are all holes, and with colors $\ge k$ are all 
single colored identical particles. Then under this projection
...
The first, most coarse
asymptotic analysis of a stochastic interacting
particle system is performed at the level of \emph{hydrodynamics}.
...
Parametrization of $(b_1,b_2)$, useful for the rest of the notes.
...
\begin{exercise}
	...
	The fact that both computations result in true identities
	of rational functions of $\rho$, $q$, and $u$, 
	is an indication of the stochastic six vertex model's integrability.
\end{exercise}
...
\begin{theorem}[\cite{aggarwal2020limit}]
	\label{thm:Burgers_for_S6V}
\end{theorem}

mention: entropy solutions, but do not elaborate.
